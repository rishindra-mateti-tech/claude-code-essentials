...
Removes: ponytail skills, taste-skill, superpowers skills, get-shit-done
commands, the rtk binary, and the pipx/npm-installed tools
(code-review-graph, graphify, markitdown, context-mode).
...
**`~/.claude/CLAUDE.md`**: the rule sections this script appended
(`## Ponytail (always use before writing code)`,
`## Taste-skill (always use before frontend/UI code)` if installed, and
`## Token efficiency (from claude-token-efficient)`) are clearly headed.
Delete those sections by hand; leave the rest of the file alone.
...
# taste-skill
rm -rf ~/.claude/skills/design-taste-frontend
